...
class HGTLayer(nn.Module):
    ...
    def reduce_func(self, nodes):
        ...
        att_img = F.softmax(nodes.mailbox['a_img'], dim=1)
        att_s = F.softmax(nodes.mailbox['a_s'], dim=1)
        l_text = nodes.mailbox['lambda_txt']
        l_img = nodes.mailbox['lambda_img']
        l_s = nodes.mailbox['lambda_s']

        # print(att.shape,l_text.shape)
        att = att * l_text + att_img * l_img + att_s * l_s



        h   = torch.sum(att.unsqueeze(dim = -1) * nodes.mailbox['v'], dim=1)
        h_img = torch.sum(att.unsqueeze(dim = -1) * nodes.mailbox['v_img'], dim=1)
        h_s = torch.sum(att.unsqueeze(dim = -1) * nodes.mailbox['v_s'], dim=1)
...
class HGT(nn.Module):
    def __init__(self, G, n_inps, n_hid, n_out, n_layers, n_heads, use_norm = True):
        ...
        for t in range(len(G.node_dict)):
            ...
            self.adapt_ws_img.append(nn.Sequential(nn.BatchNorm1d(n_inps),
                nn.Linear(n_inps, n_hid)))
            self.adapt_ws_s.append(nn.Linear(n_inps, n_hid))
    ...
    def forward(self, G, predict_type):
        for ntype in G.ntypes:
            ...
            G.nodes[ntype].data['h_s'] = torch.tanh(self.adapt_ws_s[n_id](G.nodes[ntype].data['f_s']))
        h_list=[]
        for i in range(self.n_layers):
            self.gcs[i](G, 'h', 'h','h_img','h_img','h_s','h_s')

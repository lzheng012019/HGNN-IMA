...
class HGTLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout = 0.2, use_norm = False):
        ...
        self.crossmodal = CrossModalLayer(2, out_dim,num_relations,n_heads)
        #self.num_layers = num_layers
    # def is_final_layer(self):
    #     return self.current_layer == self.num_layers - 1
    ...
    def edge_attention(self, edges):
        ...
        #beta = att *  edges.data['lambda_txt'].unsqueeze(1) + att_img * edges.data['lambda_img'].unsqueeze(1) 
        # print(beta[0])
        return {'a': att,'a_img':att_img, 'v': val,'v_img':val_img,'lambda_txt':edges.data['lambda_txt'],'lambda_img':edges.data['lambda_img']}
    
    def message_func(self, edges):
        src = edges.data['src']  
        dst = edges.data['dst']
        return {'src':src,'dst':dst,'v': edges.data['v'],'v_img':edges.data['v_img'], 'a': edges.data['a'],'a_img': edges.data['a_img'],'lambda_txt':edges.data['lambda_txt'],'lambda_img':edges.data['lambda_img']}
    
    def reduce_func(self, nodes):
        att = nodes.mailbox['a']
        att_img = nodes.mailbox['a_img']
        att_align =-torch.abs(att - att_img)
        ...
        beta1 = att * l_text + att_img * l_img
        beta = beta1 * beta2
        ...
        return {'t': h.view(-1, self.out_dim),'t_img':h_img.view(-1, self.out_dim)}
        
    def forward(self, G, inp_key, out_key,inp_key_img,out_key_img):
        ...
        for etype in G.etypes:
            edge_src, edge_dsts = G.edges(etype=etype)
            G.edges[etype].data['src']= edge_src
            G.edges[etype].data['dst'] = edge_dsts
...
class HGT(nn.Module):
    ...
    def forward(self, G, predict_type):
        for ntype in G.ntypes:
            ...
            G.nodes[ntype].data['h'] = torch.tanh(self.adapt_ws_text[n_id](G.nodes[ntype].data['f']))
            G.nodes[ntype].data['h_img'] = torch.tanh(self.adapt_ws_img[n_id](G.nodes[ntype].data['f_img']))

        h_list=[]
        for i in range(self.n_layers):
            self.gcs[i](G, 'h', 'h','h_img','h_img')
        msg = G.nodes[predict_type].data['h'].unsqueeze(dim=1)
        msg_img = G.nodes[predict_type].data['h_img'].unsqueeze(dim=1)

        msg_all = torch.cat([msg, msg_img], dim=1)#(node,64)
        w = self.project(msg_all)
        beta = torch.softmax(w, dim=1)
        h = (beta * msg_all).sum(1)
        text = self.predict(msg.squeeze(dim=1))
        img = self.predict(msg_img.squeeze(dim=1))
        out  =self.predict(h)
        return out,text,img
    def __repr__(self):

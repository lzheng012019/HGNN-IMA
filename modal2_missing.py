...
class HGTLayer(nn.Module):
    ...
    def message_func(self, edges):
        return {'v': edges.data['v'],'v_img':edges.data['v_img'], 'a': edges.data['a'],'a_img': edges.data['a_img'],'lambda_txt':edges.data['lambda_txt'],'lambda_img':edges.data['lambda_img']}
    
    def reduce_func(self, nodes):
        att = nodes.mailbox['a']*100

        att_img = nodes.mailbox['a_img']*100
...
class HGT(nn.Module):
    ...
    def forward(self, G, predict_type):
        ...
        for i in range(self.n_layers):
            s_img = self.gcs[i](G, 'h', 'h','h_img','h_img')
            epsilon = 1e-10 

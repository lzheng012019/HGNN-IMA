...
def load_g(datasets):
    ...
    elif datasets == 'AMAZON2':
        features_list,features_list_img,features_list_num,node_test, labels, train_val_test_idx, rdf, g = process.load_amazon2_data(
        train_val_test_dir='')

    features_list = [torch.FloatTensor(features).to(device) for features in features_list]
    features_list_img = [torch.FloatTensor(features).to(device) for features in features_list_img]
    features_list_num = [torch.FloatTensor(features).to(device) for features in features_list_num]
    ...
    for i,ntype in enumerate(g.ntypes):
        ...
        g.nodes[ntype].data['f_s'] = features_list_num[i]
...
def main():
    ...
    if args.type=='late':
        HGT = model_modal3.HGT
    elif args.type=='modal3':
        HGT = model_modal3.HGT
    elif args.type=='modal3_align':
        HGT = modal3_align.HGT
    ...
    for epoch in range(args.epoch):
        ...
        logits = model(g, g.predict_ntype)
        logits= logits.to(torch.float32)
        labels=labels.to(torch.float32)

        train_loss = (loss_fcn(logits[train_idx], labels[train_idx].long()) + loss_fcn(text[train_idx], labels[train_idx].long()) + loss_fcn(image[train_idx], labels[train_idx].long()) + loss_fcn(num[train_idx], labels[train_idx].long()))/4.0
        ...
        with torch.no_grad():
            logits= model(g, g.predict_ntype)
            val_loss = loss_fcn(logits[val_idx], labels[val_idx].long()) #+ loss_fcn(text[val_idx], labels[val_idx].long()) + loss_fcn(image[val_idx], labels[val_idx].long())+ loss_fcn(num[val_idx], labels[val_idx].long()))/4.0
